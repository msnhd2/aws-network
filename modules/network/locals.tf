...
locals {
    ...
    #db_subnets = sort([ for subnet in var.vpc_configuration.subnets : subnet.name if contains(subnet.name, "db") && subnet.public == false ])
    #pod_subnets = sort([ for subnet in var.vpc_configuration.subnets : subnet.name if subnet.name == "pod" && subnet.public == false ])
